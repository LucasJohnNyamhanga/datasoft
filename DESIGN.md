...
- Mobile/tablet (≤900px): message over map, sharing the same space rather than stacked in separate bands — a separate illustration band pushed the CTA below the fold, and a solid full-width card would have hidden almost all of the map either way. The card is dropped (transparent, no border) and the scrim strengthened to a fuller, more uniform dark wash so the map still visibly reads (streets, nodes, the HQ cone) while text stays unambiguously foreground.
