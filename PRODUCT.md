...
- **Maroon retired sitewide (user, 2026-08-10):** the maroon "Brief Us" closing register is dropped in favor of a near-black navy in the same blue family as the sapphire shell, so the whole palette reads as one consistent, familiar blue system — the register keeps its exact job (Contact + Footer only, same V-notch seam), just recolored. Every other maroon usage found sitewide — the process-section network-pattern animation (`app/components/NetworkPattern.tsx`, shared by the "Jinsi Tunavyofanya Kazi" / "how we work" process section on the homepage and every service page), card accent colors, glow effects, gradients — moved to the existing cobalt/navy tokens rather than a new hue. See DESIGN.md for the resulting token values.
